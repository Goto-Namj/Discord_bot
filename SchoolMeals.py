...
def crawling_split(time, re_pattern):
    ...
    for i in items:
        ...
        if str_i.startswith('<div>' + time['day'] + '<'): # <div>31< 식으로 날짜 구분
            i_list = re_pattern['amp;'].findall(str_i) # 치킨&맥주 와 같이 한 음식이 치킨&amp;맥주 로 표기되어서
            l_t_s = list_to_str(i_list)
            lts_split = l_t_s.split('[') # [조식]뭐뭐[석식]뭐뭐 식으로 구별되어있는 것을 나눔
            del lts_split[0] # [조식] 전의 필요 없는 첫 문자 제거
            menu['breakfast'] = re_pattern['korean'].findall(lts_split[0][2:])
            menu['lunch'] = re_pattern['korean'].findall(lts_split[1][2:])
            menu['dinner'] = re_pattern['korean'].findall(lts_split[2][2:])
            break
    return menu
...
class SMBot(discord.Client):
    ...
    def embed_setting(self, menu, author):
        ...
        self.embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/645939779665657866/648692896744210433/9k.png")
        self.embed.set_author(name=author, icon_url=author.avatar_url)
        self.embed.set_footer(text="남진명", icon_url="https://cdn.discordapp.com/attachments/645939779665657866/648692002384248843/54a94f8f98bd7264.png")
